import { STATUS_ORDER, statusIndex } from './status.js'

// ── Demo seed data ───────────────────────────────────────────────────────────
// Mirrors client/src/data/seed.js so the API and the offline client agree.
...
export function buildSeedReports(nowMs) {
  ...
  return RAW.map(([category, title, description, village, district, status, priority, votes, daysAgo, needs, uni, ind], i) => {
    ...
    return {
      ...
      photos: [],
      location: {
        method: 'manual', village, block: village, district,
        state: 'Jharkhand', label: `${village}, ${district}, Jharkhand`, lat: null, lng: null,
      },
      status,
      priority,
      votes,
